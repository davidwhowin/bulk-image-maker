...
CREATE POLICY "Prevent admin status changes by non-admins" ON public.user_profiles
  FOR UPDATE USING (
    auth.uid() = id AND (
      -- Allow if not changing admin status
      (SELECT is_admin FROM public.user_profiles WHERE id = auth.uid()) = is_admin
      OR
...
-- Uncomment and update the line below with your user ID after creating your account
-- INSERT INTO public.user_profiles (id, user_tier, is_admin) VALUES ('your-user-id-here', 'enterprise', TRUE) ON CONFLICT (id) DO UPDATE SET is_admin = TRUE;