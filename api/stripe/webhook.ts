...
async function handleCheckoutCompleted(session: Stripe.Checkout.Session) {
  ...
  try {
    ...
    // Also update billing_info table to store customer ID for future reuse
    ...
    console.warn(`Subscription activated for user ${userId} with tier ${tier}`);
  } catch (error) {
...
async function handleSubscriptionCreated(subscription: Stripe.Subscription) {
  ...
  try {
    ...
    console.warn(`Subscription created for user ${userId}`);
  } catch (error) {
...
async function handleSubscriptionDeleted(subscription: Stripe.Subscription) {
  try {
    ...
    console.warn(`Subscription canceled for user ${subscriptionData.user_id}`);
  } catch (error) {
...
async function handlePaymentSucceeded(invoice: Stripe.Invoice): Promise<void> {
  ...
  console.warn(`Payment succeeded for subscription: ${invoice.subscription}`);
